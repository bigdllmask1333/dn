...
use app\cms\model\Adtype as AdtypeModel;
use think\Db;
/**
 * 域名类型控制器
 * @package app\cms\admin
 */
class Adtype extends Admin
{
    /**
     * 域名类型列表
    ...
    public function index()
    {
        ...
        $order = $this->getOrder();
        // 数据列表
        $data_list = AdtypeModel::where($map)->order($order)->paginate();
...
//                ['istab', '设为导航', 'switch'],
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($page = AdtypeModel::create($data)) {
                // 记录行为
...
//                return ZBuilder::make('table')->setPageTips('这是页面提示信息')->fetch();
                ...
                $page_title = AdtypeModel::where('id', 'in', $ids)->column('title');
    ...
    public function quickEdit($record = [])
    {
        ...
        $page    = AdtypeModel::where('id', $id)->value($field);
